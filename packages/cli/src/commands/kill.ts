import type { Command } from "commander";
import chalk from "chalk";
import { DiskStorage, transition, isTerminal } from "@openingday/core";

export function registerKill(program: Command): void {
  program
    .command("kill")
    .description("Force the project into a failed state")
    .action(async () => {
      const storage = new DiskStorage(".openingday");
      if (!(await storage.exists())) {
        console.log(chalk.red("No project found. Run `openingday init --from <spec>` first."));
        return;
      }

      const state = await storage.readProjectState();
      if (isTerminal(state.status)) {
        console.log(chalk.yellow(`Project is already in terminal state "${state.status}".`));
        return;
      }

      if (state.status === "idle") {
        console.log(chalk.yellow(`Project is idle — nothing to kill.`));
        return;
      }

      const updated = transition(state, "failed");
      await storage.writeProjectState(updated);
      console.log(chalk.red("Project killed. State set to failed."));
    });
}
